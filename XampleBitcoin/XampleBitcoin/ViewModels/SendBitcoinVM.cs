...
{
    public class SendBitcoinVM : BaseViewModel
    {
        ...
        public SendBitcoinVM(INavigation navigation, BitcoinViewModel model)
        {
        ...
        public async void ConvertUSD_toBTC()
        {
            await Device.InvokeOnMainThreadAsync(async () =>
            {
                ...
                if (usd_amount != null)
                {
                    ...
                    {
                        var nk = item;
                        var bk = nk.ToString().Replace("USD", "").Replace(":", "").Replace(@"""", "").Replace(@"\", "").Replace(@"/", "");
                        var jh = bk;
                        jhk = Decimal.Parse(jh); // BTC price in USD

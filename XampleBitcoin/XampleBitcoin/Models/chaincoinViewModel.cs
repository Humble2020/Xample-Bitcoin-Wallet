...
using Xamarin.Forms;
using static XampleBitcoin.src.walletData;

namespace XampleBitcoin.src
{
    public class chaincoinViewModel : BaseViewModel
    {
        public chaincoinViewModel(INavigation Navigation, string Password, string walletPath)
        {
        ...
        public chaincoinViewModel Model { get; set; }
       ...
       public static bool CheckForInternetConnection()
        ...
        public async void Update()
        {
            ...
            foreach (var addr in data.addresses.change)
                if (!data.usedAddresses.Contains(addr))
                {
                    var address = BitcoinAddress.Create(addr, NBitcoin.Altcoins.Chaincoin.Instance.Mainnet);

...
namespace XampleBitcoin.src
{
    internal static class walletData
    {
        ...
        public static Dictionary<string, Tuple<decimal, decimal>> GetBalances(Data walletData, int minConfirmation = 1)
        {
        ...
        public static List<NB.Coin> GetUnspentCoins(Data walletData, bool JustConfirmed = true)//just added network
        {
            var UnspentCoins = new List<NB.Coin>();
            foreach (var outp in walletData.unspent_Outputs)
            {
                var address = NB.BitcoinAddress.Create(outp.address, Network.Main);
                var hash = new NB.uint256(outp.hash);
                var amount = new NB.Money(outp.value, NB.MoneyUnit.BTC);
                if (outp.confirmations < 1 && JustConfirmed)
                    continue;
